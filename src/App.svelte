...
import { insert, map, propEq } from 'ramda'
...
let insert_block = (evt) => {
  blocks = insert (add (1) (indexOf (evt.detail) 
                                    (map (prop ('id')) (blocks))))
                  ({ id: uuidv4 (), code: '', result: '' })
                  (blocks)
  console.log (blocks)
}

// TODO: Set focus on first block !!!

// TODO: Put new block on its correct position, not just the end !!!

</script>

<button on:click={insert_block}>Do</button>

<div class="w-screen h-screen">
  {#each blocks as uid }
    <Block id={uid} on:new_block={insert_block}/>
    <!--svelte:component this={ Block }
                      id={ id }
                      on:new_block={ insert_block }/-->
  {/each }
</div>
